...
namespace RTS
{
		public static class WorkManager
		{
				public static Rect CalculateSelectionBox (Bounds selectionBounds, Rect playingArea)
				{
						...
						float selectBoxLeft = playingArea.height - (screenBounds.center.y + screenBounds.extents.y);
						float selectBoxRight = screenBounds.center.x - screenBounds.extents.x;
				...
				public static WorldObject FindNearestWorldObjectInListToPosition (List<WorldObject> objects, Vector3 position)
				{
						...
						for (int i =1; i < objects.Count; i++) {
								...
								if (distanceToNearestObject < distanceToObject)
