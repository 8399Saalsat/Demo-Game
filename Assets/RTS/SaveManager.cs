...
namespace RTS
{
		public static class SaveManager
		{
				...
				{
						//needs to be adapted for terrain once implemented
				...
				private static void SaveCamera (JsonWriter writer)
				{
						...
						WriteVector (writer, "Position", Camera.main.transform.position);
						WriteQuaternion (writer, "Rotation", Camera.main.transform.rotation);
						WriteVector (writer, "Scale", Camera.main.transform.localScale);
			
						writer.WriteEndObject ();

				}

				private static void SaveResource (JsonWriter writer)
				{
				...
				public static void WriteString (JsonWriter writer, string name, string entry)
				{
						...
						writer.WritePropertyName (name);
						//make sure NotifyFilters bracketed values get stored (i.e. Refinery(Clone) should = Refinery
						if (entry.Contains ("("))
